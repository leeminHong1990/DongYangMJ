	...
	def createRoom(self, create_dict):
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, {"game_mode":0, "game_max_lose":999999, "pay_mode":pay_mode})
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "DongYang RoomID:{}".format(room.roomID))  # reason 必须为英文
