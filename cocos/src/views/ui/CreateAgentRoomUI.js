    ...
    initCreateRoom:function(){
        ...
        function create_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                var base_score = parameters["base_score"];
                var play_list = parameters["play_list"];
                var round_max_lose = parameters["round_max_lose"];
                var game_max_lose = parameters["game_max_lose"];
                var game_round = parameters["game_round"];
                var hand_prepare = parameters["hand_prepare"];
                var pay_mode = parameters["pay_mode"];
				cc.log("CreateAgentRoom args: ", parameters);
				h1global.entityManager.player().createRoom(game_round, hand_prepare, pay_mode, 1);
            }
        }
