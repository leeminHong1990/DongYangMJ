...
var GameRoomUI = UIBase.extend({
	...
	init_player_tile_panel:function(){
        ...
        function hand_tile_event(tile, touch_mode){
            ...
            if (self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE && !player.canDiscardIdx(tile.index)){ //限制出牌
            	return
			}
		...
		function player_hand_panel_event(sender, eventType){
			...
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
				...
				if (self.curSelectTile && !self.moving_tile && self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE && p.y - self.curSelectTile.getPositionY() > 76) { // 向上拖 生成一个 麻将子
					if (player.canDiscardIdx(self.curSelectTile.index)){ // 限制出牌
	...
	update_kingtile_panel:function(){
		if(!this.is_show || h1global.entityManager.player().curGameRoom.kingTiles.length <= 0){
			return;
		}
		var player = h1global.entityManager.player();
    ...
    show_operation_select_panel:function(op_btn){ // 操作二级面板 {49:[[75],[77]], 50:[[78]]}  {49:[[75],[77]]}
		...
		// 排序一下
        op_list.sort(function (a,b) {
			if(a[0] > b[0]){
				return a[0]-b[0]
			}else{
				return Math.min(a[1]) - Math.min(a[2])
			}
        });
